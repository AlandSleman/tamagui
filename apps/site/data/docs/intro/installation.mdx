...
We have more in depth guides for [Next.js](https://tamagui.dev/docs/guides/next-js), [Expo](https://tamagui.dev/docs/guides/expo) and [Vite](https://tamagui.dev/docs/guides/vite). Note that while Tamagui generally doesn't require any special bundler setup, React Native Web and the ecosystem sometimes does, so we do recommend a few configurations for the bundlers:
